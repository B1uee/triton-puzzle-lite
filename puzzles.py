...
def flashatt_kernel(
    ...
    z = tl.zeros((B0,), dtype=tl.float32) # z其实只代表分子了
    ...
    for id_j in tl.range(0, T, B1):
        ...
        # 以上部分与softmax求分母的部分相同
        # 外层求和的j与分母无关，只与分子的k_j和v_j有关
 
        # 把分子分母分开累加，但是每次缩放和累加都保证同步，最后再一起归一化，结果也是对的
        v = tl.load(v_ptr + off_j, mask=mask_j, other=0.0)
        z = z * factor + tl.sum(qk_exp * v[None, :], axis=1) # 分子也是这个公式
